...
def train(model, device, train_loader, optimizer, epoch, batchsize):
        model.train()
        for batch_idx, data in enumerate(train_loader):
                data = torch.Tensor(data).to(device).requires_grad_()
                optimizer.zero_grad()
                reconstruction, mu, logvar = model(data)
                loss, _, _ = loss_bVAE(reconstruction, data, mu, logvar, epoch)
                loss.backward(retain_graph=True)


def test(model, device, test_loader, epoch, batchsize):
        model.eval()
        losses=0
        with torch.no_grad():
                for batch_idx, data in enumerate(test_loader):
                        ...
                        reconstruction, mu, logvar = model(data)
                        loss, bce, kld = loss_bVAE(reconstruction, data, mu, logvar, epoch)
                        losses+=loss
