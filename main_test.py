...
def main():
        ...
        for epoch in range( 1,num_epochs+1 ):
                if epoch%(num_epochs//num_load)==0 or epoch==1:
                    ...
                    logger.info('memory_allocated:%.2f Mb, max_memory_allocated:%.2f Mb, memory_cached:%.2f Mb, max_memory_cached:%.2f Mb.' %(float(torch.cuda.memory_allocated()/1024**2),float(torch.cuda.max_memory_allocated()/1024**2), float(torch.cuda.memory_cached()/1024**2), float(torch.cuda.max_memory_cached()/1024**2) ))
