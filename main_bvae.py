...
def loss_bVAE(recon_x, x, mu, logvar, epoch):
    loss_mse = nn.MSELoss()
    MSE = loss_mse(recon_x,x)
    weight = 1.8 + 0.001*epoch
    KLD = -0.5 * torch.mean(1 + logvar - mu.pow(2) - logvar.exp())
    return MSE + weight*KLD, MSE, weight*KLD

def loss_bVAE2(recon_x, x, mu, logvar, epoch):
    loss_mse = nn.MSELoss()
    MSE = loss_mse(recon_x,x)
    weight = 1.5 + 0.001*epoch
    C = 0.001*epoch
...
def train2(model, D, device, train_loader, optimizer, optimizer2, epoch, batchsize):
        ...
        for batch_idx, data in enumerate(train_loader):
                ...
                if epoch%10 != 0:
                        ...
                        loss, _, _ = loss_bVAE2(reconstruction, data, mu, logvar, epoch)
                        ...
                        reconstruction, mu, logvar = model(data)
                        score = D(data)
                        loss1, _, _ = loss_bVAE2(reconstruction, data, mu, logvar, epoch)
...
def test2(model, D, device, test_loader, epoch, batchsize):
        model.eval()
        losses,losses_VAE,losses_BCE=0,0,0
        with torch.no_grad():
                for batch_idx, data in enumerate(test_loader):
                        data = torch.Tensor(data).to(device)
                        reconstruction, mu, logvar = model(data)
                        score = D(data)
                        loss1, bce, kld = loss_bVAE2(reconstruction, data, mu, logvar, epoch)
                        loss2, bce2, kld2 = loss_BCE(score, torch.ones(batchsize,1).to(device), mu, logvar, epoch)
                        loss = loss1 + 0.2*loss2
                        losses+=loss
                        losses_VAE+=loss1
                        losses_BCE+=loss2
        return losses.cpu().numpy(), losses_VAE.cpu().numpy(), losses_BCE.cpu().numpy()
...
def main():
        ...
        for epoch in range( 1,num_epochs+1 ):
                ...
                if epoch%(num_epochs//40)==0:
                        losses,losses_VAE,losses_BCE = test2(model, D, device, test_loader, epoch, batchsize)
                        logger.info( "Epoch %6d. Loss %5.3f. Loss_bVAE %5.3f. Loss_BCE %5.3f." % ( epoch, losses, losses_VAE, losses_BCE ) )
